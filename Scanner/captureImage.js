...
function attach_webcam(deviceId,index){
  ...
  function scanner_snap() {
   ...
   var image_1 = document.querySelector("#scanner_webcam").src;
    ...
    $('.alert-success').hide().removeClass('d-grid');

  }


const snap_button = document.querySelector('#snapShot'); //button for capturing the image in scanner camera
const openCamera_perRoom = document.querySelector('#open_camera_perRoom'); //button for capturing the image in scanner camera
  ...
  let camera = 0; //index for used camera
    ...
    snap_button.addEventListener('click',function(){
      ...
      if(attendance === 'out'){
        ...
        scanner_snap();
        $('#room_selection').css('display','flex');
      }
    })

    var room_arr = ['Room 1', 'Room 2', 'Room 3'];

    for(const x of room_arr){
    ...
    openCamera_perRoom.addEventListener('click',function(){
      index++
      // camera++
      let camera = $('#Room').val();
      console.log(camera);
